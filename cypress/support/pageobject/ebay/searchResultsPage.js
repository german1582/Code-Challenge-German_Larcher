...
class SearchResultsPage {
    ...
    elements = {
        ...
        generic_element: (element) => cy.xpath(element)
    }
    ...
    //Este assert puede hacerse mucho más genérico para que valide el texto de cualquier element.
    validate_search(input_string) {
        this.elements.lbl_number_of_findings().should('be.visible').then(($span) => {
            const findingsText = $span.text()
            expect(findingsText).contain(input_string)
        })
    }

    validate_element_is_visible(element) {
        this.elements.generic_element(element).should('be.visible')
